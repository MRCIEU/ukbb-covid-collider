...
### Incomplete models for generating weights

Suppose that the analysis of interest is the effect of `age` on `bmi` and we control for `sex`

```
bmi ~ age + sex
```

the weights need to include the influences of all factors in the model on the probability of being in the sample (being `tested` in this case). To illustrate let's generate a simple set of weights that include only the exposure and the outcome e.g.

```{r}
mod8 <- glm(tested ~ bmi + age, data=dat, family="binomial")
coefficients(summary(mod8)) %>% kable
prs4 <- fitted.values(mod8)
ipw4 <- ifelse(dat$tested==1, p/prs4, (1-p)/prs4)
```

or another set of weights that includes `age`, `sex` and `bmi`

```{r}
mod9 <- glm(tested ~ bmi + age + sex, data=dat, family="binomial")
coefficients(summary(mod9)) %>% kable
prs5 <- fitted.values(mod9)
ipw5 <- ifelse(dat$tested==1, p/prs5, (1-p)/prs5)
```

The effect estimate of age on bmi:

```{r}
summary(lm(bmi ~ age + sex, data=dat)) %>% coefficients %>% kable
```

Now in the tested subset:

```{r}
summary(lm(bmi ~ age + sex, data=dat, subset=tested==1)) %>% coefficients %>% kable
```

With simple weights:

```{r}
summary(lm(bmi ~ age + sex, data=dat, subset=tested==1, weight=ipw4)) %>% coefficients %>% kable
```

Full weights:

```{r}
summary(lm(bmi ~ age + sex, data=dat, subset=tested==1, weight=ipw5)) %>% coefficients %>% kable
```

```{r}
summary(lm(bmi ~ age + sex, data=dat, subset=tested==1, weight=ipw3)) %>% coefficients %>% kable
```
...
- In practice an outcome of interest might be COVID test positive vs test negative. In this case, we only have test results in the subset, so we cannot treat it as a nested sample unless we assume that our selection model captures the influence of testing positive on being included in the sample. Therefore, performing the non-nested sensitivity analyses would be important.
